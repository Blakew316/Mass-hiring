...
// Canada. Without these every Canadian number fell through to Central, which
// means a Vancouver number would have been texted at 7am local.
addZone('902 782 428', 'America/Halifax');                 // Nova Scotia, PEI
addZone('506', 'America/Moncton');                         // New Brunswick
addZone('709', 'America/St_Johns');                        // Newfoundland
addZone('418 438 450 514 579 581 819 873 263 354 367 468 367', 'America/Toronto');   // Quebec keeps Eastern time
addZone('226 249 289 343 365 416 437 519 548 613 647 683 705 742 807 905 942', 'America/Toronto');
addZone('204 431 584', 'America/Winnipeg');                 // Manitoba
addZone('306 474 639', 'America/Regina');                   // Saskatchewan, no daylight saving
addZone('368 403 587 780 825', 'America/Edmonton');         // Alberta
addZone('236 250 257 604 672 778', 'America/Vancouver');    // British Columbia
addZone('867', 'America/Whitehorse');                       // the territories
ZONES.set('907', 'America/Anchorage');
ZONES.set('808', 'Pacific/Honolulu');
const DEFAULT_ZONE = 'America/Chicago';

// Toll-free ranges, per the North American Numbering Plan (including the ones
// reserved for future toll-free use, so this does not need revisiting).
...
function validNanp(ten) {
  ...
  // Toll-free and premium-rate lines pass every structural test above but are
  // switchboards, IVRs and paid lines — never somebody's mobile. Texting one
  // spends a send from a very small daily allowance on a phone system.
  if (TOLL_FREE.has(area) || area === '900' || area === '976') return false;
  return true;
}
