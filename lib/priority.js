...
// The answer depends on nothing but the job title, the employer and whatever
// they did before, and a list of several thousand people holds far fewer
// distinct combinations than it does people — a few hundred, typically. This
// is called once per candidate every time the dashboard asks for state, and
// each call walks a dozen keyword lists, so remember what it worked out.
// Frozen because the result is shared between callers.
const industryCache = new Map();
const INDUSTRY_CACHE_MAX = 20000;   // far above any real list; a bound, not a policy

function industry(c) {
  const past = Array.isArray(c.pastRoles) ? c.pastRoles.join('\u0001') : (c.pastRoles || '');
  const key = `${c.role || ''}\u0000${c.company || ''}\u0000${past}`;
  const known = industryCache.get(key);
  if (known) return known;
  const out = Object.freeze(classify(c));
  // A crude bound: this only grows with distinct job/employer pairs, so it
  // reaching the ceiling means something unexpected is happening. Start over
  // rather than hold memory a serverless instance cannot spare.
  if (industryCache.size >= INDUSTRY_CACHE_MAX) industryCache.clear();
  industryCache.set(key, out);
  return out;
}
