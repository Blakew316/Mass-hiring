...
async function seedTeam() {
  const existing = await storage.getJson('db');
  if (existing) throw new Error('That team already has data — refusing to overwrite it.');
  await storage.setJson('db', blankTeamDocument());
  return true;
}

module.exports = { load, save, update, addEvent, pushEvent, clearEvents, addToThread, addCalendlyKey, seedTeam, blankTeamDocument, THREAD_MAX, CALENDLY_KEYS_MAX, rid, DEFAULT_TEMPLATE, DEFAULT_FOLLOW_UP, DEFAULT_TEXT_TEMPLATE, NEW_TEAM_TEXT_TEMPLATE, FEED_TYPES, EVENT_CHANNEL };
