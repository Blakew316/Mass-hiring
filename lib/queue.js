...
const MIN_SEND_DEADLINE_MS = 4000;  // a send is still started late in the run if it gets at least this long
const WRITE_RESERVE_MS = 1500;      // time kept for the writes after the last send
const SEND_GAP_MS = 250;            // breathing space between one worker's sends
...
function normalizePaceSettings(settings) {
  ...
  norm('dailyLimit', MIN_DAILY, MAX_DAILY_WORKSPACE);
...
async function processQueue({ budgetMs = 20000, maxSends } = {}) {
  ...
  const pause = (kind, until, why) => {
    ...
    if (pauseUntil && kind !== 'daily') return;
    if (kind === 'daily' && pauseKind === 'daily') return;
  ...
  const sendOne = async (item, left) => {
    ...
    if (unv) {
      ...
      try {
        ...
        if (found) { await markSent(c, found); return; }
      } catch (err) {
    ...
    try {
      ...
      if (kind === 'daily') {
        pause('daily', new Date(Date.now() + 60 * 60 * 1000), 'Gmail reports its daily sending limit is reached — sending resumes automatically.');
        return;
      }
  ...
  // Workers pull the next planned item until the plan, the budget or a halt ends the run.
  let cursor = 0;
  const worker = async (index) => {
    if (index) await sleep(index * 150);   // stagger the first sends a little
    for (;;) {
      if (halt) return;
      const left = budgetMs - (Date.now() - started);
      if (left < MIN_SEND_DEADLINE_MS + WRITE_RESERVE_MS) return;
      if (cursor >= n) return;
      ...
      if (cursor < n && !halt) await sleep(SEND_GAP_MS);
    }
  };
  ...
  const workers = clamp(Math.ceil(n / 6), 1, st.via === 'gmail-api' ? MAX_WORKERS : 2);
...
module.exports = {
  ...
  SEND_TIMEOUT_MS, TIMEOUT_UNKNOWN, MAX_PER_MINUTE, DEFAULT_PER_MINUTE, MAX_DAILY_WORKSPACE, MAX_DAILY_GMAIL,
};
