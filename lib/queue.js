...
async function processQueue({ budgetMs = 20000, maxSends } = {}) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (followUp && c.status !== 'emailed') { done.add(item.id); continue; }   // answered or booked meanwhile
...
module.exports = {
  loadQ, saveQ, updateQ, limits, enqueue, clearQueue, retryFailed, recordSent, deferUnverified, applySentPatch, status, processQueue,
