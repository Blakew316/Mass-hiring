...
const MAX_PENDING = 5000;           // unsettled sends kept (a normal run settles its own within seconds)
...
const QUEUE_DEFAULTS = {
  ...
  // [{ id, lastEmailedAt, gmailThreadId, messageId, lastSubject, followUp }]:
  // sends recorded here but not yet written onto the candidate. They go in
  // with the send itself, so a run cut off before it updates the candidate
  // list cannot leave anyone looking never contacted; settlePending() below
  // finishes the job, on the next run if not this one.
  pendingPatches: [],
...
function normalize(raw) {
  ...
  q.pendingPatches = (Array.isArray(q.pendingPatches) ? q.pendingPatches : [])
    .filter((p) => p && p.id && p.lastEmailedAt).slice(-MAX_PENDING);
  return q;
}
...
// The same send can be applied twice — a run and the next one both settling
// it — and an older send can arrive after a newer one was written, so it never
// counts a follow-up twice or winds the candidate back to an older email.
function applySentPatch(c, p) {
  const at = Date.parse(p.lastEmailedAt);
  if (c.status === 'new') c.status = 'emailed';
  if (p.followUp && !(Date.parse(c.lastFollowUpAt) >= at)) {
    c.followUpCount = (c.followUpCount || 0) + 1;
    c.lastFollowUpAt = p.lastEmailedAt;
  }
  if (Date.parse(c.lastEmailedAt) > at) return;
...
// Write recorded sends onto their candidates, then drop them from the queue.
// Safe to repeat and to run from two places at once (see applySentPatch).
const patchKey = (p) => `${p.id}\n${p.lastEmailedAt}`;
async function settlePending(list) {
  ...
  await store.update((db) => {
    ...
    for (const p of list) {
      ...
      if (!c) continue;   // deleted since: nothing to update
  ...
  const settled = new Set(list.map(patchKey));
  await updateQ((f) => {
    const keep = f.pendingPatches.filter((p) => !settled.has(patchKey(p)));
    if (keep.length === f.pendingPatches.length) return false;
    f.pendingPatches = keep;
  });
  return list.length;
}
...
// Record a send (either path) so both share one truth. `patch` is what the
// send changes on the candidate, kept until settlePending() has written it.
function recordSent(q, id, email, ts = new Date().toISOString(), patch = null) {
  q.sentLog.push({ id, email, ts });
  if (patch) q.pendingPatches.push({ ...patch, id, lastEmailedAt: patch.lastEmailedAt || ts });
...
async function processQueue({ budgetMs = 20000, maxSends } = {}) {
  ...
  // A run cut off after sending, before it updated the candidates, left those
  // updates here: finish them first, so nobody it emailed looks never contacted.
  if (q.pendingPatches.length) {
    try { await settlePending(q.pendingPatches); }
    catch (err) { console.warn('[queue] earlier sends not written to candidates yet:', err.message); }
  }
    ...
    chain = chain.then(async () => {
      ...
      try {
        latest = await updateQ((f) => {
          for (const e of batch) { recordSent(f, e.id, e.email, e.ts, e.patch); f.sent += 1; }
  ...
  const markSent = async (c, sent, { followUp = false, subject = '' } = {}) => {
    ...
    await persistSent({ id: c.id, email: c.email, ts, patch });
  };
  ...
  q = await updateQ((f) => {
    for (const e of unsaved) { recordSent(f, e.id, e.email, e.ts, e.patch); f.sent += 1; }
    f.total = Math.max(f.total, f.sent);
  ...
  // Every send is in the queue's pending list by now (written with the send
  // itself); write them onto the candidates. Should this fail or the run be
  // cut off here, the next run does it.
  if (q.pendingPatches.length) {
    try { await settlePending(q.pendingPatches); }
    catch (err) { console.warn('[queue] sends not written to candidates yet, retried next run:', err.message); }
  }
...
module.exports = {
  ...
  applySentPatch, settlePending, status, processQueue, classifySendError, retryAfterFrom, sentToday, sentInLastMinute, recentlySentIds, sendWithDeadline,
