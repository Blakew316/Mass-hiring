...
function loginLockedFor(req, teamId) {
  const a = bucket(keysFor(req, teamId).ip);
  return a.lockedUntil > Date.now() ? Math.ceil((a.lockedUntil - Date.now()) / 1000) : 0;
}

function recordLoginFailure(req, teamId) {
  ...
  if (ip.count >= MAX_FAILS) ip.lockedUntil = Date.now() + LOCK_MS;
...
function clearLoginFailures(req, teamId) {
  const k = keysFor(req, teamId);
