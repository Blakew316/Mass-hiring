...
// against, the secret that signs that team's session cookies, and fingerprints
// of the relay token and the Sales IQ token so a bearer token can be traced
// back to its team. A team's actual work -- candidates, templates, settings,
// queues, Gmail tokens -- lives under its own storage keys and is unreachable
...
function normalizeTeam(raw) {
  ...
  return {
    ...
    // SHA-256 of the Sales IQ connection token. Its own field, never shared
    // with the relay's: each token is looked up only against its own
    // fingerprints, which is what keeps one from opening the other's routes.
    // Every registry write goes through here, so a field left out of this
    // list is erased the next time anybody renames a team or sets a PIN.
    salesiqTokenHash: typeof raw.salesiqTokenHash === 'string' ? raw.salesiqTokenHash : '',
  };
}
...
// ---------- the Sales IQ token ----------
// The same arrangement as the relay's, for a different reader: the Sales IQ
// hiring dashboard, which may see who has booked an interview and nothing
// else. An empty token clears the fingerprint, and with it every copy of the
// connection code that was ever handed out.
async function setSalesiqToken(id, token) {
  return edit(id, (t) => { t.salesiqTokenHash = token ? fingerprint(token) : ''; });
}

async function teamForSalesiqToken(token) {
  const fp = fingerprint(token);
  for (const t of await all()) {
    if (t.salesiqTokenHash && safeEqual(t.salesiqTokenHash, fp)) return t;
  }
  return null;
}
...
module.exports = {
  ...
  setRelayToken, teamForRelayToken, setSalesiqToken, teamForSalesiqToken, fingerprint, randHex,
};
