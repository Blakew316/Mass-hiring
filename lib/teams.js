...
function checkPinLength(pin) {
  ...
  if (p.length < MIN_PIN) throw new Error(`Choose a PIN of at least ${MIN_PIN} characters.`);
  if (p.length > MAX_PIN) throw new Error('That PIN is too long.');
  return p;
}
...
function verifyPin(team, pin) {
  ...
  return safeEqual(hashPin(given, team.pin.salt), team.pin.hash);
}
...
async function create({ name, pin }, seed) {
  ...
  await storage.updateJson(KEY, (raw) => {
    ...
    const id = uniqueId(slugify(clean), new Set(reg.teams.map((t) => t.id)));
...
module.exports = {
  KEY, LEGACY_ID, LEGACY_NAME, MIN_PIN, MAX_NAME, MAX_TEAMS,
  all, byId, publicList, edit, create, remove,
  verifyPin, usesAppPassword, pinRecord, checkPinLength, cleanName, slugify,
