...
// Unwritten outcomes kept (a report normally writes its own at once).
...
const DEFAULTS = {
  ...
  // [{ id, status, body, ts }]: what the relay reported, not yet written onto
  // the candidate. Stored with the report itself, so a request cut off before
  // it updates the candidate list cannot leave a texted candidate looking
  // untexted; the next report or claim writes it (settleTextOutcomes, app.js).
  pendingPatches: [],
  total: 0,
  sent: 0,
  startedAt: null,
...
function normalize(raw) {
  ...
  q.pendingPatches = (Array.isArray(q.pendingPatches) ? q.pendingPatches : [])
    .filter((o) => o && o.id && o.ts).slice(-MAX_PENDING);
  return q;
}
...
function enqueue(q, db, ids, template, { ignoreQuietHours = false } = {}) {
  ...
  for (const id of ids) {
    ...
    if (!c) continue;
    if (queued.has(id) || leased.has(id)) {
      // Asking to send now for someone already waiting is a request to unstick
      // them, not a duplicate. Release the parked time, mark it urgent and move
      // it to the front, rather than reporting that nothing happened.
      const waiting = ignoreQuietHours ? q.items.find((i) => i.id === id) : null;
      if (waiting) {
        waiting.now = 1;
        waiting.at = null;
        if (t) waiting.t = t;
        q.items = [waiting, ...q.items.filter((i) => i !== waiting)];
        promoted += 1;
      } else {
        skipped.queued += 1;
      }
  ...
  q.note = '';
  pruneTemplates(q);
  return { added: added.length, promoted, skipped };
}

function enqueueReply(q, candidate, body) {
  const p = phone.normalize(candidate && candidate.phone);
  if (!p) return { ok: false, reason: 'no-phone' };
  if (q.optOut.includes(p)) return { ok: false, reason: 'opted-out' };
  const text = String(body || '').trim();
  if (!text) return { ok: false, reason: 'empty' };
  const t = templateKey({ body: text });
  q.templates[t] = { body: text };
  if (!q.items.length && !Object.keys(q.leases).length) q.startedAt = new Date().toISOString();
  q.items.unshift({ id: candidate.id, phone: p, t, at: null, now: 1, reply: 1 });
  q.total += 1;
  q.note = '';
...
// Outcomes now written onto their candidates leave the pending list.
function dropPending(q, list) {
  const key = (o) => `${o.id}\n${o.ts}`;
  const done = new Set((list || []).map(key));
  const keep = q.pendingPatches.filter((o) => !done.has(key(o)));
  if (keep.length === q.pendingPatches.length) return false;
  q.pendingPatches = keep;
}
...
function claim(q, db, { now = Date.now(), render } = {}) {
  ...
  for (let i = 0; i < q.items.length; i++) {
    ...
    // The parked time is only ever a cache of a decision made under the
    // settings in force when it was parked. Changing the hours has to release
    // it, so the window is always re-judged against the CURRENT settings and
    // the stamp is refreshed or cleared — a stale stamp outliving the setting
    // that produced it is indistinguishable from the queue being broken.
    const inWindow = it.now ? true : phone.withinHours(it.phone, { startHour, endHour, days }, at);
    if (!inWindow) {
      const parkUntil = phone.nextWindowStart(it.phone, { startHour, endHour, days }, at).toISOString();
      if (it.at !== parkUntil) { it.at = parkUntil; changed = true; }
      quiet += 1; continue;
    }
    if (it.at) { it.at = null; changed = true; }   // the window opened; let it go
    const tpl = (it.t && q.templates[it.t]) || null;
    // A campaign body is a template and gets placeholders filled. A reply is
    // literally what was typed into the thread, braces and all.
    const raw = String((tpl && tpl.body) || '');
    const body = it.reply || !render ? raw : render(tpl, c);
...
function report(q, { jobId, status, error }) {
  ...
  delete q.leases[jobId];
  const outcome = { id: lease.id, status: status === 'sent' || status === 'not-imessage' ? status : 'failed', ts: new Date().toISOString() };
  if (status === 'sent' && lease.body) outcome.body = lease.body;
  q.pendingPatches.push(outcome);
  if (status === 'sent') {
    ...
    return { ok: true, status: 'sent', candidateId: lease.id, phone: lease.phone, body: lease.body || '' };
  }
...
module.exports = {
  ...
  enqueue, enqueueReply, clearQueue, retryFailed, recordSent, fail, addOptOut, reap, claim, report, dropPending, status,
