...
function buildMime({ from, to, subject, text, html, attachments = [], messageId, inReplyTo, references }) {
  ...
  const lines = [...headers, `Content-Type: multipart/mixed; boundary="${mixed}"`, '', `--${mixed}`, `Content-Type: multipart/alternative; boundary="${alt}"`, '', ...body];
  for (const a of attachments) {
    const fname = encodeWord(String(a.filename || 'attachment').replace(/["\r\n]/g, ''));
    lines.push(
      `--${mixed}`,
...
async function gmailSend(settings, { to, subject, html, text, attachments = [], threadId, inReplyTo, references, messageId }, { signal } = {}) {
  const token = await accessToken(settings);
  if (!token) throw new Error('Google is not connected.');
  const t = (await loadTokens()) || {};
  const mid = messageId || newMessageId(t.email || '');
  const raw = buildMime({ from: fromHeader(settings.fromName, t.email || 'me'), to, subject, text, html, attachments, messageId: mid, inReplyTo, references });
...
async function findSentTo(settings, email, sinceMs) {
  ...
  const data = await res.json().catch(() => ({}));
  if (!res.ok) {
    const msg = (data.error && data.error.message) || `Gmail search failed (${res.status})`;
    const e = new Error(msg);
    e.status = res.status;
    if (res.status === 403 || /insufficient|scope/i.test(msg)) e.scope = true;
    throw e;
...
module.exports = { authUrl, exchangeCode, status, clearTokens, fetchSheetRows, gmailSend, buildMime, newMessageId, getSignature, threadReplies, threadMessages, cleanReply, classifyReply, findSentTo, baseUrl, accessToken };
