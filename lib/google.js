...
function buildMime({ from, to, subject, text, html, attachments = [] }) {
  ...
  const headers = [`From: ${from}`, `To: ${to}`, `Subject: ${encodeWord(subject)}`, 'MIME-Version: 1.0'];
  const body = [
...
// messages only.
async function gmailSend(settings, { to, subject, html, text, attachments = [] }, { signal } = {}) {
  const token = await accessToken(settings);
  if (!token) throw new Error('Google is not connected.');
  const t = (await loadTokens()) || {};
  const raw = buildMime({ from: fromHeader(settings.fromName, t.email || 'me'), to, subject, text, html, attachments });
...
module.exports = { authUrl, exchangeCode, status, clearTokens, fetchSheetRows, gmailSend, buildMime, getSignature, threadReplies, cleanReply, classifyReply, findSentTo, baseUrl };
