...
// Has anyone other than the sender written in this Gmail thread? Uses only
// message headers (gmail.metadata). err.scope is set when the connected
// account has not granted that permission.
async function threadHasReply(settings, threadId, myEmail) {
  const token = await accessToken(settings);
  if (!token) throw new Error('Google is not connected.');
  const res = await fetch(
  ...
  for (const m of data.messages || []) {
    const from = ((m.payload && m.payload.headers) || []).find((h) => h.name.toLowerCase() === 'from');
    const addr = from ? String(from.value).toLowerCase() : '';
    if (addr && me && !addr.includes(me)) return { replied: true, from: from.value };
  }
  return { replied: false };
}

module.exports = { authUrl, exchangeCode, status, clearTokens, fetchSheetRows, gmailSend, getSignature, threadHasReply, baseUrl };
