...
function sendStatus(settings) {
  const g = google.status(settings);
  ...
  return { ready: false, via: null, from: null };
}

async function sendEmail(settings, message) {
  const st = sendStatus(settings);
  if (!st.ready) {
  ...
  await transport.sendMail({
    from: settings.senderName ? `"${settings.senderName}" <${user}>` : user,
    to: message.to,
