...
async function getBytes(key) {
  ...
  try { return fs.readFileSync(path.join(DATA_DIR, `${key}.bin`)); } catch { return null; }
}
...
async function setBytes(key, buffer) {
  ...
  fs.mkdirSync(DATA_DIR, { recursive: true });
  fs.writeFileSync(path.join(DATA_DIR, `${key}.bin`), buffer);
}
...
async function del(key) {
  ...
  try { fs.unlinkSync(path.join(DATA_DIR, `${key}.bin`)); } catch {}
}
...
module.exports = { getJson, setJson, getJsonWithEtag, setJsonIfMatch, updateJson, getBytes, setBytes, del, backend, onNetlify };
