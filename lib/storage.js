...
//
// Every write carries a one-off marker in the blob's metadata. The Blobs
// client reports ANY answer to a conditional write other than 412 as a
// success — a 500 or a 403 included — so a write that comes back without an
// ETag is checked against the stored marker before anyone is told it saved.
async function setJsonIfMatch(key, value, etag) {
  key = scopedKey(key);
  const s = backing();
  if (s) {
    try {
      const mark = crypto.randomBytes(8).toString('hex');
      const put = async (cond) => {
        const r = await s.setJSON(key, value, { ...cond, metadata: { w: mark } });
        if (r.modified === false) return false;
        if (r.etag) return true;
        const meta = await s.getMetadata(key);
        if (meta && meta.metadata && meta.metadata.w === mark) return true;
        throw new Error('the save could not be confirmed');
      };
      if (await put(etag ? { onlyIfMatch: etag } : { onlyIfNew: true })) return true;
      // Rejected. A real concurrent change shows a different ETag now, and the
      // caller re-reads and re-applies its change on top of it. The HEAD is not
      // allowed to fail quietly: a stale "missing" read plus a failed check must
      // never add up to "safe to overwrite".
      const meta = await s.getMetadata(key);
      const current = meta ? (meta.etag || null) : null;
      if ((current || null) !== (etag || null)) return false;
      if (!current) return false;   // gone since, yet onlyIfNew failed: re-read
      // Rejected although the stored version is the one we read. Try again,
      // still conditional, on the tag exactly as the store reports it now
      // (and without a weak W/ prefix a proxy may have added).
      ...
      if (!again || again.etag !== current) return false;
      // The condition itself is misfiring. Last resort, as before: write, and
      // say so in the function log.
      console.warn(`[storage] conditional write on "${key}" rejected with an unchanged ETag; writing unconditionally`);
      const r = await s.setJSON(key, value, { metadata: { w: mark } });
