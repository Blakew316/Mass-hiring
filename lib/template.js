...
function fill(text, candidate, settings) {
  const vars = {
    ...
    originalSubject: candidate.lastSubject || '',
  };
...
// One iMessage for one candidate. Same {{placeholders}} as the email, but no
// HTML, no signature and no tracking pixel — a text is just text. The booking
// link goes on its own line at the end, bare, because that is what a phone
// turns into something tappable.
function renderText(template, candidate, settings) {
  let body = fill((template && template.body) || '', candidate, settings)
    .replace(/\r\n/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim();
  const calendly = String((settings && settings.calendlyUrl) || '').trim();
  if (calendly && !body.includes(calendly)) body += `\n\n${calendly}`;
  return body;
}

module.exports = { renderEmail, renderText, fill, signatureText, escapeHtml };
