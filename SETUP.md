...
2. **Candidates**: tick the people to contact → **Email selected** (or the ✉ on one row). Review the message → **Send**. Edits you make in this window apply to this batch only, not to the saved template. Each person gets their own individual email from your work address, sent about a second apart; each line turns ✅ as it lands, a toast reads *Sent N of N emails*, and the window closes. Failures show ❌ with the reason and the button becomes **Retry failed** (re-sends only those). Gmail caps outgoing mail at roughly 500/day for a personal Gmail and 2,000/day for Workspace (less for a brand-new account) — above that, sends fail with a limit/quota error; send the rest the next day.
3. Statuses update automatically (*Not contacted → Emailed* on the first send; *Booked* on a Calendly booking; back to *Emailed* on a cancellation). To change one by hand, pick it from the dropdown in the Status column. Re-emailing someone already marked Replied or Booked leaves their status alone.
