...
| Some rows were skipped on import | Their email cell wasn't a single valid address (blank, several addresses in one cell, or stray characters). The import result counts them; fix those rows in the sheet and import again — duplicates are skipped, so re-importing is safe. |
