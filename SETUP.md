...
3. **Set the admin password** (the app refuses to run publicly without one): Netlify → **Project configuration → Environment variables → Add a variable** (older Netlify UI: *Site configuration*) → key `APP_PASSWORD`, value = a strong password, leave *Scopes* and *Deploy contexts* at their defaults → **Create variable**. Then **Deploys → Trigger deploy → Deploy site** and wait until the deploy shows *Published*.

   This one password does two jobs: it is what lets you create and delete teams, and it is how the first team — **Team Maverick**, which owns everything stored before teams existed — signs in until you give it four digits of its own in Settings → Team. Every team you make after that signs in with its own four-digit PIN and sees only its own candidates.
...
2. **Webhook (booking alerts)** — this is what flips a candidate to *Booked* and pings your phone. Calendly delivers webhooks on its paid plans (**Standard, Teams, Enterprise** — not Free).
   ...
   3. The app calls Calendly's API to subscribe your Netlify URL (`/webhooks/calendly`) to `invitee.created` and `invitee.canceled`. It generates a random signing key, registers it with the subscription, and stores it so every incoming event is verified. The token is kept (masked, like your other keys) so the app can also **sync your scheduled interviews**: every few minutes it pulls upcoming events from Calendly, matches invitees to candidates by email and marks them *Booked*; the **Interviews booked** tile on the Dashboard lists them all, with a *Sync now* button. Clicking *Enable booking alerts* again later is safe — it replaces the old subscription (use this after renaming the site or if bookings stop arriving).
...
2. **Send to everyone**: on the same page, the **Send** card's button *Email all N not contacted* queues the email in the editor for everyone still marked *Not contacted*. Review the message → **Queue N emails**. The server then sends them automatically, up to 30 per minute by default (Settings → *Sending pace*, at most 60 — the Gmail API's quota), each person getting their own individual email from your work address — you can close the tab. The **Dashboard** shows a *Sending in progress* card with a progress bar, the pace and a time estimate, how many went out in the last 24 hours against your daily limit, any pause and exactly when sending resumes, and **Stop sending** / **Retry N failed** buttons. Gmail allows roughly 2,000 messages per Workspace account per rolling 24 hours (500 on free Gmail); the queue pauses at your *Daily send limit* (default 1,800, at most 2,000) and resumes by itself the moment the 24-hour window frees up — the card shows that time, and raising the limit in Settings resumes it at once. To email specific people instead, tick them on **Candidates** → **Email selected** (8 or fewer go out immediately).
3. Statuses update automatically: *Not contacted → Emailed* on send; **Replied** when a real person replies in the Gmail thread (checked every minute while the dashboard is open; also pushed to your phone); **Bounced** when Gmail reports the address doesn't exist ("Address not found", "Undeliverable"); *Booked* on a Calendly booking; back to *Emailed* on a cancellation. Out-of-office and other automatic replies never count as a reply. Opens are recorded via an invisible image in the email and listed in **Dashboard → Candidate updates**. To change a status by hand, pick it from the dropdown in the Status column.
...
| "Set the admin password first" screen | `APP_PASSWORD` isn't set in Netlify (or the deploy that picks it up hasn't finished) — Step 0.3. |
| "Choose your team" on an installed app that used to just ask for a password | A second team now exists, and the copy of the app on that device is older than teams. Close it, reopen it, and it will pick up the new version by itself. |
...
| Google: **Error 400: redirect_uri_mismatch** ("This app sent an invalid request") | The address the app sends Google is not on your OAuth client. Settings → Google shows it under *Authorized redirect URI* with a **Copy** button (it follows your site's primary domain — after adding a custom domain it changes, e.g. from `https://<site>.netlify.app/auth/google/callback` to `https://wpioutreach.com/auth/google/callback`). In Google Cloud Console → APIs & Services → Credentials → your OAuth client → **Authorized redirect URIs** → **Add URI** → paste → **Save**. Keep the old address too if you still open the dashboard there. Google can take a minute to apply it; then click Connect / Reconnect again. |
...
| Replied tile says *Reply text hasn't been captured yet* | That reply was detected before the app had Gmail read permission, so only the sender and time were stored. After **Reconnect** the app re-reads those threads a few at a time and fills the text in within a couple of minutes. If it says *can't be read with the current Google permissions*, reconnect and tick every box. |
| I want a flyer / PDF sent with every email | Email Template → **Attachments** → *Add file* (PNG, JPG, GIF, WebP or PDF, up to 4 MB each, three files). The Account Executive flyer is already there; click × to remove it, and *Restore the Account Executive flyer* to put it back. Attachments show in the preview and the send dialog. |
| Apollo answers *not included in your plan* | The key is valid but the Apollo account behind it does not sell API access (trial and free plans do not). Make the key on an Apollo account whose plan includes the API, or add candidates by importing a CSV exported from Apollo. The card now says which of the two problems it is. |
| The Apollo card says *API key needed* | Settings → Apollo → paste an API key from Apollo (Settings → Integrations → API), then save. Searching is free; revealing an address costs one Apollo credit. |
| An Apollo pull added fewer people than it revealed | People already in your candidate list are not added twice (their blank details are filled in instead), records with no address are skipped, and anyone at your own company is never added. The result line breaks down exactly what happened. |
| The dashboard says *daily limit of N reached* and sending is paused for hours | That is the *Daily send limit* you set (Settings → Sending pace), not Gmail refusing: the queue counts every email of the last 24 hours and waits until the oldest is a day old. Raise the limit (up to 2,000 on Google Workspace, Google's own ceiling) and sending continues immediately; beyond that, a bigger list simply takes another day. |
| The dashboard says *Gmail itself reports the account has reached its daily sending limit* | That one is Google refusing, not your setting: the account has sent its 2,000 (or 500) for the rolling day. Raising the *Daily send limit* will not lift it; sending resumes by itself once Google allows it again. |
| Settings shows a smaller *Emails per minute* than I typed | Values above 60 are stored as 60 — the Gmail API accepts about 60 sends a minute (6,000 quota units, 100 per send). The dashboard and Settings always show the same, real pace. |
| The Follow up button says *No follow-ups due* | Nobody has been waiting long enough yet, or everyone has had the maximum number already. Settings → Sending pace → *Follow up after (days)* and *Follow-ups per person* control this; the Emailed tile marks who is due. |
| "Nothing new to import" / 0 added | Everyone in that file is already in your candidate list (the import page says so before you click, and lists a few of them). That is normal when re-importing a master list. Tick *Fill in blank details* to copy any phone, company or location the file has that the list is missing. |
| Rows "without a usable email" | Listed by row number with the offending cell before you import. Usually a blank cell, two addresses in one cell, or a typo like `name at gmail dot com`. Fix them in the sheet and import again — nobody is added twice. |
| My file will not open (Numbers, .xls, .ods) | Export it as .xlsx or CSV from that app; the import reads CSV, TSV, TXT and .xlsx, in any encoding. You can also copy the cells and paste them into the box on the Import page. |
| Interviews tile says *not in your candidate list* for someone who is | They booked with a different name **and** address than the sheet has. Click **Link to candidate** on that row, type part of their name or email, and pick them — the app remembers the address. (A different address alone, or a different spelling of the name alone, is matched automatically.) |
| Phone test says *Cannot convert argument to a ByteString* | Fixed — pushes are sent as JSON now, so emoji and accented names in titles work. Click **Send test** again; the warning clears on success. |
| Replied count went down after an update | Bounces and automatic replies used to count as replies. They are now filtered out (bounces become **Bounced**) and the wrongly-flagged candidates and feed lines are cleaned up automatically. |
