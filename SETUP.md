...
2. **Send to everyone**: on the same page, the **Send** card's button *Email all N not contacted* sends the email in the editor to everyone still marked *Not contacted*. Review the message → **Send**. Each person gets their own individual email from your work address; it goes out in batches of 8 with a progress bar — keep the tab open until it finishes (you can press **Stop** between batches). Failures show ❌ with the reason and the button becomes **Retry N failed**. To email specific people instead, tick them on **Candidates** → **Email selected**. Gmail caps outgoing mail at roughly 500/day for a personal Gmail and 2,000/day for Workspace (less for a brand-new account) — above that, sends fail with a limit/quota error; send the rest the next day.
